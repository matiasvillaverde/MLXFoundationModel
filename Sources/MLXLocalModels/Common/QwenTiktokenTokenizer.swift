...
internal final class QwenTiktokenTokenizer: @unchecked Sendable, Tokenizer {
    internal static let vocabFilename = "qwen.tiktoken"
    ...
    internal let bosTokenId: Int? = 151_643
    internal let eosToken: String? = "<|endoftext|>"
    internal let eosTokenId: Int? = 151_643
    ...
    internal init(vocabURL: URL, modelMaxLength: Int = 8_192) throws {
    ...
    internal func applyChatTemplate(
        ...
        if let tools, !tools.isEmpty {
            throw Tokenizers.TokenizerError.chatTemplate("Qwen tokenizer does not define a tool-use template")
        }
        if additionalContext?.isEmpty == false {
            throw Tokenizers.TokenizerError.chatTemplate("Qwen tokenizer does not use additional template context")
        }
    ...
    private func validate(_ argument: ChatTemplateArgument?) throws {
        ...
        case .literal(let template):
            guard template == Self.defaultChatTemplate else {
                throw Tokenizers.TokenizerError.chatTemplate(
                    "Qwen tokenizer only supports its default chat template"
                )
            }
        case .name(let name):
    ...
    private func render(messages: [Message], addGenerationPrompt: Bool) throws -> String {
        ...
        for message in messages {
            ...
            result += "<|im_start|>\(role)\n\(Self.stringContent(from: message["content"]))<|im_end|>\n"
        }
    ...
    private static func makeSpecialTokens() -> [String: Int] {
        ...
        for index in 0 ..< 205 {
            tokens["<|extra_\(index)|>"] = 151_646 + index
        }
        return tokens
    }
...
private struct QwenTokenizerConfig: Decodable {
