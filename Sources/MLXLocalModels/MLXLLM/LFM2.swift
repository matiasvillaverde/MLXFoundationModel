...
internal struct LFM2Configuration: Codable, Sendable {
    ...
    var fullAttnIdxs: [Int] {
        if let _fullAttnIdxs {
            return _fullAttnIdxs
        }
        if let layerTypes {
            return layerTypes.enumerated().compactMap { index, type in
                type.contains("attention") ? index : nil
            }
        }
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.vocabularySize =
            try container.decodeIfPresent(Int.self, forKey: .vocabularySize) ?? 65_536
...
private class ShortConv: Module {
    ...
    init(_ args: LFM2Configuration, layerIdx: Int) {
        self.args = args
