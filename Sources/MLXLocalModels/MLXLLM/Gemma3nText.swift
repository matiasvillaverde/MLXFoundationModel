...
private class Gemma3nAttention: Module {
    ...
    func callAsFunction(
        ...
        queries = queries.reshaped(batchSize, sequenceLength, -1, headDim)
        ...
        if isKvSharedLayer, let cache, let state = materializedKVState(from: cache) {
            let keys = state.keys
            ...
            let adjustedMask = adjustedMask(
                mask,
                keySequenceLength: keys.shape[keys.shape.count - 2],
            ...
            var keys: MLXArray
            var values: MLXArray
            keys = kProj(input).reshaped(batchSize, sequenceLength, -1, headDim)
            keys = kNorm(keys)
            keys = keys.transposed(0, 2, 1, 3)
            keys = rope(keys, offset: offset)

            values = vProj(input).reshaped(batchSize, sequenceLength, -1, headDim)
            values = vNorm(values)
            ...
            let result = attentionWithCacheUpdateReturningKV(
                ...
                maskForKeySequenceLength: { keySequenceLength in
                    self.adjustedMask(
                        mask,
                        keySequenceLength: keySequenceLength,
                        queryDType: queries.dtype
                    )
                }
            )
    ...
    private func adjustedMask(
        ...
        guard case .array(let maskArray) = mask else { return mask ?? .none }
        if maskArray.shape.last != keySequenceLength {
            return .array(maskArray[.ellipsis, 0 ..< keySequenceLength].asType(queryDType))
        }
        return .array(maskArray.asType(queryDType))
    }
}
...
private class MLP: Module {
    ...
    func callAsFunction(_ input: MLXArray) -> MLXArray {
        let gateProj = self.gateProj(input)
        ...
        let upProj = self.upProj(input)
        return self.downProj(activations * upProj)
    }
...
private class Gemma3nAltUp: Module {
    ...
    func computeRouterModalities(_ input: MLXArray) -> MLXArray {
        let routerInputs = routerNorm(input) * _routerInputScale.asType(routerNorm.weight.dtype)
        let routed = modalityRouter(routerInputs).asType(.float32)
        return tanh(routed)
    }

    func predict(_ input: MLXArray) -> MLXArray {
        let modalities = computeRouterModalities(input[config.altupActiveIdx])
        ...
        return finalPredictions.asType(input.dtype)
    }
    ...
    func callAsFunction(_ input: MLXArray, activated: MLXArray) -> (MLXArray, MLXArray) {
        let predictions = predict(input)
...
private class Gemma3nDecoderLayer: Module {
    ...
    func callAsFunction(
        ...
        if isSliding, case .array(let maskArray) = mask {
            ...
            let disabledMaskValue = attentionMaskDisabledValue(matching: maskArray)

            let slidingWindowMask = tril(
                MLXArray.ones(maskArray.shape, dtype: .bool),
                k: -slidingWindow
            )
            let updatedMask = MLX.where(slidingWindowMask, disabledMaskValue, maskArray)
        ...
        let predictions = altup.predict(input)
...
private class Gemma3nLanguageModel: Module {
    let config: Gemma3nTextConfiguration
    let hiddenSize: Int
    let vocabSize: Int
    let vocabSizePerLayerInput: Int
    ...
    func newCache(parameters: GenerateParameters?) -> [any KVCache] {
        ...
        let slidingWindow = config.slidingWindow > 0 ? config.slidingWindow : 4_096
        let firstKvSharedLayerIdx = config.numHiddenLayers - config.numKvSharedLayers
        let layerTypes =
            config.layerTypes ?? Array(repeating: "global_attention", count: config.numHiddenLayers)

        for index in 0 ..< firstKvSharedLayerIdx {
            let layerType = layerTypes[index]
            if layerType == "full_attention" {
                caches.append(StandardKVCache())
            } else if layerType == "sliding_attention" {
                caches.append(RotatingKVCache(maxSize: slidingWindow, keep: 0))
            } else {
                fatalError("Unknown layer type: \(layerType) for layer \(index)")
            }
        }
        return caches
    }
    ...
    init(_ config: Gemma3nTextConfiguration) {
        ...
        for (index, layerType) in layerTypes.enumerated() {
            if index < firstKvSharedLayerIdx {
                layerIdxToCacheIdx.append(index)
            } else {
                if layerType == "full_attention" {
    ...
    func callAsFunction(
        ...
        if let inputsEmbeds {
            ...
            hiddenStates = (hiddenStates * MLXArray(_embedTokensScale, dtype: .float32)).asType(hiddenStates.dtype)
        } else {
        ...
        let finalPerLayerInputs = projectPerLayerInputs(hiddenStates, perLayerInputs: perLayerInputsProcessed)
        ...
        let cachePosition = MLXArray(pastSeenTokens ..< (pastSeenTokens + hiddenStates.shape[1]))
        ...
        if mask == nil {
            let fullCacheSlice = Array(cacheArray[firstFullIdx...]).compactMap(\.self)
            fullMask = createAttentionMask(h: hiddenStates, cache: fullCacheSlice, returnArray: true)

            let slidingCacheSlice = Array(cacheArray[firstSlidingIdx...]).compactMap(\.self)
            slidingWindowMask = createAttentionMask(
                h: hiddenStates, cache: slidingCacheSlice, returnArray: true)
        }

        let initialHidden = hiddenStates

        let targetMagnitude = pow(mean(initialHidden.square(), axis: -1, keepDims: true), 0.5)
        let epsilonTensor = MLXArray(Float.leastNormalMagnitude, dtype: initialHidden.dtype)

        var hList = Array(repeating: initialHidden, count: config.altupNumInputs)

        for index in 1 ..< config.altupNumInputs {
            let altupProj = altupProjections[index - 1](hList[index])
            hList[index] = altupProj.asType(initialHidden.dtype)
        }

        hiddenStates = stacked(hList, axis: 0)

        if config.altupNumInputs > 1 {
            let mags = pow(mean(hiddenStates[1...].square(), axis: -1, keepDims: true), 0.5)
            hiddenStates[1...] = hiddenStates[1...] * (targetMagnitude / maximum(mags, epsilonTensor))
        }

        for (index, layer) in layers.enumerated() {
            let perLayerInput = finalPerLayerInputs[0..., 0..., index, 0...]

            let layerTypes =
                config.layerTypes
                ?? Array(repeating: "global_attention", count: config.numHiddenLayers)
            let isGlobal = layerTypes[index] == "full_attention"
            ...
            let cacheIdx = layerIdxToCacheIdx[index]
        ...
        let targetMagnitudeFinal = pow(mean(hiddenStates[0].square(), axis: -1, keepDims: true), 0.5)

        for index in 1 ..< config.altupNumInputs {
            let altupUnembProj = altupUnembedProjections[index - 1](hiddenStates[index])
            hiddenStates[index] = altupUnembProj.asType(initialHidden.dtype)
        }

        if config.altupNumInputs > 1 {
            let mags = pow(mean(hiddenStates[1...].square(), axis: -1, keepDims: true), 0.5)
            hiddenStates[1...] = hiddenStates[1...] * (targetMagnitudeFinal / maximum(mags, epsilonTensor))
        }

        hiddenStates = mean(hiddenStates, axis: 0)
        var out = norm(hiddenStates)
...
internal class Gemma3nTextModel: Module, LLMModel {
    ...
    public func callAsFunction(_ inputs: MLXArray, cache: [KVCache]?) -> MLXArray {
        callAsFunction(inputs, inputsEmbeds: nil, mask: nil, cache: cache)
    }
...
extension Gemma3nTextModel: LoRAModel {
    public func loraLinearLayers() -> LoRALinearLayers {
        languageModel.layers.map { layer in
