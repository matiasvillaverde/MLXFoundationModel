...
private class Attention: Module {
    ...
    init(_ args: Starcoder2Configuration) {
        self.args = args
...
internal struct Starcoder2Configuration: Codable, Sendable {
    var hiddenSize: Int
    var hiddenLayers: Int
    var intermediateSize: Int
    var attentionHeads: Int
    var kvHeads: Int
    var maxPositionEmbeddings: Int = 16_384
    var normEpsilon: Float = 1e-5
    var normType: String = "layer_norm"
    var vocabularySize: Int = 49_152
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.maxPositionEmbeddings =
            try container.decodeIfPresent(
                Int.self, forKey: Self.CodingKeys.maxPositionEmbeddings) ?? 16_384
        self.normEpsilon =
            try container.decodeIfPresent(
                Float.self, forKey: Self.CodingKeys.normEpsilon) ?? 1e-5
        self.normType =
            try container.decodeIfPresent(
                String.self, forKey: Self.CodingKeys.normType) ?? "layer_norm"
        self.vocabularySize =
            try container.decodeIfPresent(
                Int.self, forKey: Self.CodingKeys.vocabularySize) ?? 49_152
        self.ropeTheta =
            try container.decodeIfPresent(
        ...
        self.tieWordEmbeddings =
            try container.decodeIfPresent(
                Bool.self, forKey: Self.CodingKeys.tieWordEmbeddings)
            ?? true
    }
}
