...
private class Attention: Module {
    ...
    var queryProjection: Linear
    @ModuleInfo(key: "k_proj")
    var keyProjection: Linear
    @ModuleInfo(key: "v_proj")
    var valueProjection: Linear
    @ModuleInfo(key: "o_proj")
    var outputProjection: Linear
    ...
    init(_ args: Qwen2Configuration) {
        ...
        _queryProjection.wrappedValue = Linear(dim, heads * headDim, bias: true)
        _keyProjection.wrappedValue = Linear(dim, kvHeads * headDim, bias: true)
        _valueProjection.wrappedValue = Linear(dim, kvHeads * headDim, bias: true)
        _outputProjection.wrappedValue = Linear(heads * headDim, dim, bias: false)

        let ropeScale: Float
        if !args.ropeScaling.isEmpty, args.ropeScaling["type"] == .string("linear"),
            let factor = args.ropeScaling["factor"] {
            if let floatValue = factor.asFloat() {
                ropeScale = 1 / floatValue
            } else {
...
private class MLP: Module, UnaryLayer {
    @ModuleInfo(key: "gate_proj")
    var gate: Linear
    @ModuleInfo(key: "down_proj")
    var down: Linear
    @ModuleInfo(key: "up_proj")
    var upProjection: Linear
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        down(silu(gate(x)) * upProjection(x))
    }
}
...
internal class Qwen2Model: Module, LLMModel, KVCacheDimensionProvider, GreedyTokenModel {
...
internal struct Qwen2Configuration: Codable, Sendable {
    var hiddenSize: Int
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.kvHeads = try container.decode(Int.self, forKey: Self.CodingKeys.kvHeads)
        self.ropeTheta =
