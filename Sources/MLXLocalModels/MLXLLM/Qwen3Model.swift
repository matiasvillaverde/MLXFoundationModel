...
private class Attention: Module {
    ...
    @ModuleInfo(key: "q_norm")
    var qNorm: RMSNorm
    @ModuleInfo(key: "k_norm")
    var kNorm: RMSNorm
    ...
    init(_ args: Qwen3Configuration) {
        ...
        _queryProjection.wrappedValue = Linear(dim, heads * headDim, bias: false)
        _keyProjection.wrappedValue = Linear(dim, kvHeads * headDim, bias: false)
        _valueProjection.wrappedValue = Linear(dim, kvHeads * headDim, bias: false)
...
internal struct Qwen3Configuration: Codable, Sendable {
    var hiddenSize: Int
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.headDim = try container.decode(
            Int.self, forKey: Self.CodingKeys.headDim)
