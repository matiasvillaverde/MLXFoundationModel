...
internal struct HeliumConfiguration: Codable, Sendable, Equatable {
    ...
    internal init(from decoder: Decoder) throws {
        ...
        self.init(
            modelType: try container.decodeIfPresent(String.self, forKey: .modelType) ?? "helium",
            ...
            rmsNormEps: try container.decodeIfPresent(Float.self, forKey: .rmsNormEps) ?? 1e-8,
            vocabularySize: try container.decode(Int.self, forKey: .vocabularySize),
            maxPositionEmbeddings: try container.decodeIfPresent(
                Int.self,
                forKey: .maxPositionEmbeddings
            ) ?? 4_096,
            ropeTheta: try container.decodeIfPresent(Float.self, forKey: .ropeTheta) ?? 100_000,
...
internal struct HeliumAttentionLayout: Sendable, Equatable {
    ...
    internal init(_ config: HeliumConfiguration) {
        precondition(config.hiddenSize > 0, "Helium hidden_size must be positive")
        precondition(config.attentionHeads > 0, "Helium num_attention_heads must be positive")
        precondition(config.kvHeads > 0, "Helium num_key_value_heads must be positive")
        ...
        precondition(
            ...
            "Helium attention heads must be divisible by KV heads"
        )
        precondition(config.resolvedHeadDim > 0, "Helium head_dim must be positive")
...
private final class HeliumAttention: Module {
    private let layout: HeliumAttentionLayout
    ...
    init(_ config: HeliumConfiguration) {
        let layout = HeliumAttentionLayout(config)
        self.layout = layout
        self.rope = RoPE(
...
private final class HeliumFeedForward: Module, UnaryLayer {
...
private final class HeliumDecoderLayer: Module {
    @ModuleInfo(key: "self_attn") fileprivate var selfAttention: HeliumAttention
    @ModuleInfo(key: "mlp") private var feedForward: HeliumFeedForward
    ...
    init(_ config: HeliumConfiguration) {
        _selfAttention.wrappedValue = HeliumAttention(config)
        _feedForward.wrappedValue = HeliumFeedForward(config)
...
private final class HeliumBackbone: Module {
    @ModuleInfo(key: "embed_tokens") fileprivate var tokenEmbeddings: Embedding
    @ModuleInfo(key: "layers") fileprivate var layers: [HeliumDecoderLayer]
    @ModuleInfo(key: "norm") private var finalNorm: RMSNorm

    init(_ config: HeliumConfiguration) {
        precondition(config.vocabularySize > 0, "Helium vocab_size must be positive")
        precondition(config.hiddenLayers > 0, "Helium num_hidden_layers must be positive")
...
internal final class HeliumModel: Module, LLMModel, KVCacheDimensionProvider, GreedyTokenModel {
    internal let vocabularySize: Int
    internal let kvHeads: [Int]
    internal let configuration: HeliumConfiguration

    @ModuleInfo(key: "model") private var backbone: HeliumBackbone
    @ModuleInfo(key: "lm_head") private var lmHead: Linear?

    init(_ configuration: HeliumConfiguration) {
        ...
        _backbone.wrappedValue = HeliumBackbone(configuration)
