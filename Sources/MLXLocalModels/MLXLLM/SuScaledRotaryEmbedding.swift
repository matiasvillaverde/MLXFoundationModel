...
internal class SuScaledRotaryEmbedding: Module {
    let dimensions: Int
    ...
    public func callAsFunction(_ x: MLXArray, offset: Int = 0) -> MLXArray {
        ...
        let scaledX: MLXArray
        if x.dim(-1) == dimensions {
            scaledX = scale * x
        } else {
            let rotatedHead = scale * x[.ellipsis, 0 ..< dimensions]
            let tail = x[.ellipsis, dimensions...]
            scaledX = concatenated([rotatedHead, tail], axis: -1)
        }
