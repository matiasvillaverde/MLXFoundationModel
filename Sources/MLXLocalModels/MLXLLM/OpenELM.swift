...
private class MultiHeadCausalAttention: Module {
    ...
    init(_ args: OpenElmConfiguration, layerId: Int) {
...
private class FeedForwardNetwork: Module, UnaryLayer {
    ...
    init(_ args: OpenElmConfiguration, layedId: Int) {
...
class OpenELMModelInner: Module {
    ...
    init(_ args: OpenElmConfiguration) {
...
internal struct OpenElmConfiguration: Codable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.modelType = try container.decode(
            String.self, forKey: Self.CodingKeys.modelType)
        self.headDimensions = try container.decode(
            Int.self, forKey: Self.CodingKeys.headDimensions)
        self.numTransformerLayers = try container.decode(
            Int.self, forKey: Self.CodingKeys.numTransformerLayers)

        self.modelDim = try container.decode(
            Int.self, forKey: Self.CodingKeys.modelDim)
        self.vocabularySize = try container.decode(
            Int.self, forKey: Self.CodingKeys.vocabularySize)
        self.ffnDimDivisor = try container.decode(
            Int.self, forKey: Self.CodingKeys.ffnDimDivisor)
        ...
        let queryDims = qkvMultipliers.map { a in
            makeDivisible(Float(modelDim) * a, divisor: headDimensions * headMultipleOf)
        }

        self.numQueryHeads = queryDims.map { qDim in
            Int(computeHeads(modelDim: qDim, headDim: headDimensions))
        }
        ...
        self.ffnWithGlu =
            try container.decodeIfPresent(
                Bool.self, forKey: Self.CodingKeys.ffnWithGlu) ?? true
        self.normalizeQkProjections =
            try container.decodeIfPresent(
                Bool.self, forKey: Self.CodingKeys.normalizeQkProjections) ?? true
        self.shareInputOutputLayers =
            try container.decodeIfPresent(
                Bool.self, forKey: Self.CodingKeys.shareInputOutputLayers) ?? true
    }
}
