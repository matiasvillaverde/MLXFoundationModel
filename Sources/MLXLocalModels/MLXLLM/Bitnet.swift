...
/// Thread-safe singleton manager for BitLinear Metal kernel
///
/// This class is marked `@unchecked Sendable` because:
/// - It contains the `bitlinearKernel` which is a Metal kernel object
/// - The kernel is initialized once in `init()` and never modified afterward
/// - Access is only through the singleton `shared` instance
/// - Metal kernels are inherently thread-safe for read-only access
///
/// Safety guarantees:
/// - Immutable after creation: The kernel is created once and never modified
/// - Singleton pattern: Only one instance exists via `shared`
/// - Thread-safe reads: Multiple threads can safely access the read-only kernel
/// - No mutable state: The class contains no mutable properties
...
internal struct BitnetConfiguration: Codable, Sendable {
