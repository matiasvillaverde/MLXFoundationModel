...
@Suite("MLX required tool grammar")
struct MLXRequiredToolGrammarBuilderTests {
    ...
    func genericStylesKeepSchemaRequiredToolGrammar() {
        let grammar = MLXRequiredToolGrammarBuilder.grammar(
            from: [Self.weatherTool],
            promptStyle: .chatML
        )
    ...
    func xmlNativeGrammarsConstrainScalarValuesFromToolSchema() throws {
        let countGrammar = MLXRequiredToolGrammarBuilder.grammar(
        ...
        let enabledGrammar = MLXRequiredToolGrammarBuilder.grammar(
    ...
    @Test("DeepSeek DSML native grammars mark scalar parameter string encoding")
    func deepSeekDSMLNativeGrammarsMarkScalarParameterStringEncoding() throws {
        let countGrammar = MLXRequiredToolGrammarBuilder.grammar(
            from: [Self.weatherCountTool],
    ...
    private static var weatherCountTool: MLXBridgeToolDefinition {
        MLXBridgeToolDefinition(
            name: "weather",
            description: "Read local weather for a city",
            parametersJSONSchema: """
    ...
    private static var weatherEnabledTool: MLXBridgeToolDefinition {
        MLXBridgeToolDefinition(
            name: "weather",
            description: "Read local weather for a city",
            parametersJSONSchema: """
