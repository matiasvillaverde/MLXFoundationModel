...
@Suite("MLX GLM stream reducer")
struct MLXGLMStreamReducerTests {
    @Test("suppresses bare GLM XML while streaming visible text")
    func suppressesBareGLMXMLWhileStreamingVisibleText() throws {
        var reducer = MLXToolAwareStreamReducer(tools: [Self.weatherTool])
        var actions: [MLXToolAwareStreamReducer.Action] = []

        actions.append(contentsOf: reducer.consume(Self.textChunk("before wea")))
        actions.append(contentsOf: reducer.consume(Self.textChunk(
            #"ther<arg_key>city</arg_key><arg_value>123</arg_value>"#
        )))
        actions.append(contentsOf: reducer.consume(Self.textChunk(
            #"<arg_key>count</arg_key><arg_value>"2"</arg_value> after"#
        )))
